...
(function() {
  var bsort, canvas, checkDone, checkDoneInterval, colours, context, defer, height, hsort, index, initColours, isort, qsort, rectHeight, rectWidth, reset, sort, ssort, start, swapRects, timeouts, toHslString, width;
  ...
  isort = function() {
    var j, k, ref;
    for (j = k = ref = index; (ref <= 0 ? k < 0 : k > 0); j = ref <= 0 ? ++k : --k) {
  ...
  ssort = function() {
    var j, k, min, ref, ref1;
    min = index - 1;
    for (j = k = ref = index, ref1 = colours.length; (ref <= ref1 ? k < ref1 : k > ref1); j = ref <= ref1 ? ++k : --k) {
